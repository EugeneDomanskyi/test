...
const RaffleListBrowse = ({ onParticipate, onClaim, onShare }) => {
    ...
    <App.Flex column gap={16}>
      <App.Flex wrap gap={[32, 16]} >
        {campaigns.slice((page - 1) * perPage, page * perPage).map(item => (
          <RaffleListBrowseItem key={item.id} item={item} onParticipate={onParticipate} onClaim={onClaim} onShare={onShare} />
        ))}
      </App.Flex>
