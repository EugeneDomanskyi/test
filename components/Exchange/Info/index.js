        ...
        <>
          <App.Flex row align="center" className={styles.gap}>
            <App.Flex row center gap={8}>
              ...
              <App.Flex column gap={4}>
                <App.Flex row align="center" gap={8}>
                  ...
                  <App.Text size={12} color={current?.externalUrl ? '#4C69FF' : '#5E5C6B'} nowrap sx={{ cursor: 'pointer' }} height={1} onClick={handleClickLink('website')}>{ current?.name } {current?.externalUrl ? <App.Icon icon="external-link" /> : null}</App.Text>
                </App.Flex>
                ...
                {current?.address ? (
                  <App.Flex row align="center" gap={4}>
                    ...
                    <a href={scanLink} target="_blank" rel="noreferrer">
                      <App.Text inline size={12} color="#5E5C6B" nowrap height={1} onClick={handleClickLink(blockchain?.code)} sx={{ cursor: 'pointer' }}>{ blockchain?.name }: <b>{[current.address.slice(0, 7), current.address.slice(-7)].join('...')}</b></App.Text>
                    </a>
            ...
            <App.Flex column gap={6}>
              ...
              <App.Number size={12} height={1} color="#5E5C6B">${ usdPrice }</App.Number>
            </App.Flex>
          </App.Flex>
