...
const TournamentInfo = ({ tiers, tournament, results }) => {
  ...
  const calculateWidth = () => {
    ...
    const currentTier = tiers.find(item => item.level == level)
    const nextTier = level < tiers.length ? tiers.find(item => item.level == level + 1) : currentTier

    const step = 100 / (tiers.length - 1)
