...
const PointsProgressCircle = ({size = 200, trackWidth = 24, progress = 72, ...props}) => {
  const center = size / 2
  ...
  return (
    <App.Flex center width={size} height={size} className={styles.container}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg">
        <circle cx={center} cy={center} r={radius} stroke="#121019" fill="transparent" strokeDasharray={dashArray} strokeWidth={trackWidth} />
      ...
      <App.Flex column center className={styles.info}>
        ...
        <App.Flex center width={100}>
          <App.Text center size={14} color="#9B99AE">Successful Referrals</App.Text>
        </App.Flex>
      </App.Flex>
    </App.Flex>
  )
