...
const SwapModalComplete = ({ collection, currency, type, nfts, onComplete }) => {
  ...
  return (
    <App.Flex column>
      <App.Flex column gap={8} align="center" className={styles.content}>
        ...
        <App.Text center size={20} weight={600}>Your Swap Was Successful!</App.Text>
        ...
        <App.Flex row center gap={8} className={styles.container}>
          ...
          <App.Text color="#B9B8C5">You have swaped {nfts.length} {collection.name} NFT{nfts.length > 1 ? 's' : ''}</App.Text>
        </App.Flex>
      </App.Flex>
...
export default SwapModalComplete