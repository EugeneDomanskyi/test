      ...
      <App.Flex column>
        ...
        <App.Flex row>
          ...
          <App.Flex width={['auto', 120]} flex={[1, null]} center>
            <App.Text center weight={600} height={1} color="#A6DC37">{t(`Gems${isMobile ? '' : ' earned'}`)}</App.Text>
          </App.Flex>
              ...
              getLeaderboard().map((item, index) => (
                <App.Flex key={index} row className={styles.row}>
                  <App.Flex width={[92, 44]} center>
                    ...
                    <App.Text color={getColor(index + 1, item.reward ?? 0)} size={[16, 14]} weight={600} sx={{position: 'absolute'}}>{index + 1}</App.Text>
                  </App.Flex>
...
export default GemsHomeLeaderboard