...
const GemsReferHistory = () => {
  const { t } = useTranslation()

  const isMobile = useSelector(({ $app }) => $app.size.isMobile)
  const referrals = useSelector(({ $gem }) => $gem.referrals)
      ...
      <App.Flex column className={styles.table}>
        <App.Flex row className={styles.row}>
          ...
          <App.Flex flex={1} align="center" justify={['center', 'flex-end']} sx={{paddingRight: 8}}>
            <App.Text center weight={600} height={1} color="#A6DC37">{t('Gems earned')}</App.Text>
          </App.Flex>
        </App.Flex>
          ...
          {referrals.length ? (
            referrals.map((item, index) => (
              <App.Flex key={index} row className={styles.row}>
                ...
                <App.Flex flex={1} align="center" justify={['center', 'flex-end']} sx={{paddingRight: 8}}>
                  <App.Text center weight={[600, 400]} height={1}>{item.points} Gems</App.Text>
                </App.Flex>
              </App.Flex>
            ))
          ) : (
...
export default GemsReferHistory