...
const BotAuctionsImage = ({ item }) => {
  return (
    <App.Flex column align="center" className={styles.container}>
      <BotAuctionsBadge status={item.status} win={item.current && item.claimHash == ''} />

      <App.Flex className={cn(styles.itemImage, {[styles.gray]: item.status == 'closed' && (!item.current || item.current && item.claimHash != '')})} sx={{ backgroundImage: `url("/images/bot/image-placeholder.png")` }}>
