...
export const request = async (uri, method = 'GET', {api, jwt_token, ...data} = {}, formData = null) => {
  ...
  if (api === 'payram-dev') {
    options.headers['API-Key'] = process.env.NEXT_PUBLIC_PAYRAM_DEV_API_KEY
  }

  if (api === 'payram-preprod') {
    options.headers['API-Key'] = process.env.NEXT_PUBLIC_PAYRAM_PREPROD_API_KEY
  }
...
const getBaseUrl = (api) => {
  switch (api) {
    ...
    case 'payram-dev':
      return process.env.NEXT_PUBLIC_PAYRAM_DEV_URL
    case 'payram-preprod':
      return process.env.NEXT_PUBLIC_PAYRAM_PREPROD_URL
    default:
